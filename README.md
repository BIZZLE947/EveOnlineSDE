## EVE ONLINE SDE CONVERSION - YAML to CSV

Automatically checks *https://developers.eveonline.com/static-data* daily and converts YAML files to CSV. 

[![Latest Release](https://img.shields.io/github/v/release/BIZZLE947/EveOnlineSDE)](https://github.com/BIZZLE947/EveOnlineSDE/releases/latest)


## Assets include:
1.  **eve_sde_csv.zip**: The full dataset in CSV format.
2.  **schema_map.json**: A dictionary listing every file and its available columns.

## Download latest release
[📥 Download Latest SDE Data (CSVs)](https://github.com/BIZZLE947/EveOnlineSDE/releases/latest)

## Want to run locally?
### Prerequisites
* Python 3.10+

### Installation
1.  Clone the repository:
    ```bash
    git clone https://github.com/YOUR_USERNAME/EveOnlineSDE.git
    cd EveOnlineSDE
    ```

2.  Run the script (Dependencies auto-install):
    ```bash
    python "YAML Converter.py" "path/to/sde_folder or YAML file"
    ```

*Note: The script will automatically check for and install `pandas`, `pyyaml`, `openpyxl`, and `tqdm` if they are missing.*


Want to send some love?
IGN: BladeM5
